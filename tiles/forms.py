...
class TileSearchForm(forms.Form):
    ...
    tile_type = forms.ChoiceField(required=False, choices=[
        ('', 'All Types'),
        ('floor', 'Floor Tiles'),
        ('wall', 'Wall Tiles'),
        ('both', 'Floor & Wall'),
    ...
    country = forms.ChoiceField(required=False, choices=[('', 'All Countries')],
        widget=forms.Select(attrs={
            'class': 'rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20'
        }), label='')
    ...
    def __init__(self, *args, **kwargs):
        ...
        cats = TileCategory.objects.all()
        self.fields['category'].choices = [('', 'All Categories')] + [(c.slug, c.name) for c in cats]
        countries = Country.objects.all()
        self.fields['country'].choices = [('', 'All Countries')] + [(c.slug, c.name) for c in countries]